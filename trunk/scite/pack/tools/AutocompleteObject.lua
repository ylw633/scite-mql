...
Ввод разделителя изменяет регистр символов в имени объекта в соответствии с записью в api файле (например "ucase" при вводе автоматически заменяется на "UCase")
...
3.0 (Tymur):
	...
	* исправил старый баг, описанный в сообщении VoidVolker: http://forum.ru-board.com/topic.cgi?forum=5&topic=24956&start=840#5
	...
	+ если в api-файле Луа встречаются строчки вида: t["a-b+c"]\nэто очень хитрый параметр,
...
3.09(Tymur):
	...
	+ теперь в .api файлах допустима конструкция вида #$string_value=^'.*' и даже #$file=io.open%b(). Часть справа от знака "=" -- паттерн, используемый для распознавания объекта. Т.о. возможно расширить функциональность фичи для распознавания строк на всё, что ловится регулярными выражениями Луа.
...
local function AutocompleteObject(char)
	...
	-- Важно: запоминаем текщую позицию курсора (Иначе "string.b|[Enter]" превратится в "string.bbyte")
