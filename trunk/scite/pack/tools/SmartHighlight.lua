...
function SmartHighlight()
	...
	local matchcase  = tonumber(ifpnil('smart.highlight.matchcase',1)) > 0
	local min_len = tonumber(ifpnil('smart.highlight.minlen',1))
	local max_num = tonumber(ifpnil('smart.highlight.maxnum',100))
...
AddEventHandler("OnUpdateUI", function()
	if tonumber(ifpnil('smart.highlight',0)) < 1 --[[or editor.Length > 500000]] then
		return
	end
	SmartHighlight()
end)