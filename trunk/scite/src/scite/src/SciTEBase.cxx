...
bool SciTEBase::InsertAbbreviation(const char* data) {
	std::string tmp = EncodeString(data); //>!-add-[FixEncoding]
...
//!-start-[Zoom]
		if (extender && static_iOnSendEditorCallsCount < _MAX_SEND_RECURSIVE_CALL) {
			static_iOnSendEditorCallsCount++;
			char zoom[10];
			sprintf(zoom, "%d", (int)wEditor.Call(SCI_GETZOOM));
...
//!-start-[LangMenuChecker]
	...
	if(language != last_lang) {
		...
		for (int item = 0; item < languageMenu.size(); item++) {
			...
			if(language == props.GetNewExpandString("lexer.", fn.c_str())) {
...
bool SciTEBase::IsMenuItemEnabled(int cmd) {
	...
	case IDM_REDO:
		...
		break;
	case IDM_DUPLICATE:
		return !CurrentBuffer()->isReadOnly;
		break;
	case IDM_FINDINFILES:
